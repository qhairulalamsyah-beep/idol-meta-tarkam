...
function cmdBantuan(): CommandResult {
  ...
  lines.push("─────────────────────");
  lines.push("");
  lines.push("💡 Contoh: `!daftar Joko`");
  lines.push("💡 Contoh: `!daftar Sari female`");
  lines.push("💡 Contoh: `!daftar Joko male NEXUS` (dengan club)");
  lines.push("💡 Contoh: `!akun` untuk cek status pendaftaran");
  lines.push("💡 Contoh: `!sawer 10000 Semangat!`");
  lines.push("💡 Contoh: `!profil Joko` untuk cari profil pemain");
  lines.push("💡 Contoh: `!statistik Sari` untuk detail statistik");
  lines.push("💡 Contoh: `!club` untuk ranking club");
...
function cmdStatus(): CommandResult {
  if (!isDbAvailable()) {
    return {
      command: "!status",
      reply: "⚠️ Database tidak tersedia saat ini. Coba lagi nanti atau hubungi admin.",
    };
  }
  ...
  const lines: string[] = ["🏆 *STATUS TURNAMEN IDOL META*\n"];
  ...
  for (const t of tournaments) {
    ...
    const statusIcon = emoji[t.status] || "❓";
    const division = t.division === "male" ? "Male" : "Female";
    const week = t.week ? `Week ${t.week}` : "";
    const bracket = t.bracketType ? ` (${t.bracketType} elimination)` : "";

    lines.push(`${statusIcon} *${t.name}*`);
    lines.push(`   Division: ${division} ${week ? `| ${week}` : ""}`);
    lines.push(`   Status: ${statusLabel(t.status)}`);
    lines.push(
      `   Type: ${t.type}${bracket} | Hadiah: ${formatMoney(t.prizePool)}`
    );
    lines.push(
      `   Pemain: ${t.playerCount} | Pertandingan: ${t.matchCount}`
    );
    lines.push("");
  }
...
function cmdPemain(args: string): CommandResult {
  ...
  if (maleTournament) {
    ...
    if (maleRegs.length > 0) {
      ...
      lines.push(`\n♂ *DIVISI MALE* (${maleTournament.name})`);

      if (approved.length > 0) {
        lines.push(`✅ Disetujui (${approved.length}):`);
        for (const r of approved.slice(0, 10)) {
          const tierIcon = getTierIcon(r.tierAssigned || r.tier);
          lines.push(`  ${tierIcon} ${r.name} — ${r.tierAssigned || r.tier}`);
        }
        if (approved.length > 10) {
          lines.push(`  ... dan ${approved.length - 10} lainnya`);
        }
      }
  ...
  if (femaleTournament) {
    ...
    if (femaleRegs.length > 0) {
      ...
      lines.push(`\n♀ *DIVISI FEMALE* (${femaleTournament.name})`);
  ...
  lines.push("");
  lines.push("💡 Gunakan !pemain male atau !pemain female untuk detail per divisi.");
...
function cmdPeringkatByDivision(division: 'male' | 'female'): CommandResult {
  ...
  if (rankings.length === 0) {
    return {
      command: "!peringkat",
      reply: `📋 Belum ada data peringkat divisi ${division === 'male' ? 'Male' : 'Female'}.\n\n💡 Peringkat akan muncul setelah turnamen berlangsung dan skor dimasukkan.`,
    };
  }

  const lines: string[] = [
    `🏅 *PERINGKAT ${division.toUpperCase()}* ${divisionIcon}\n`,
  ];
  ...
  for (let i = 0; i < rankings.length; i++) {
    ...
    lines.push(
      `   ${tierIcon} Tier ${u.tier} | 🏆 ${u.points} pts`
    );
    lines.push(
...
async function cmdAkun(senderPhone: string, senderJid: string): Promise<CommandResult> {
  ...
  const user = safeGet(
    `SELECT id, name, phone, email, gender, tier, points, createdAt FROM User WHERE phone = ?`,
    [senderPhone]
  );
